...
# USAGE:
#
# sh launch_rdm_alljobs_TBW.sh
#
#------------------------------------------------------------------------------
# Step 1: User to specify main assumptions
...
gcam_meta_scenario='RDM_Policy'
#'RDM_Policy'  # scenarios upon which variations will be done.
...
py_config_gen=gcam_config_generator.py

# Not likely to require a change
...
if [[ $run_gcam -eq 1 ]]; then
	# Calculate the number of groups of x, and any remainder to be dealt with
...
#---------------------------------------------------------------------------------------------
# Step 6: Run an inventory to see which runs completed (produced output files). The script is
# likely to throw a python traceback in its current form if all runs successfully completed.
if [[ $run_inventory -eq 1 ]]; then
	...
	jid_inventory=$(sbatch -A $acct -p $partition --output=$slurmoutname_serial $InventoryFpath $repo_path $gcam_meta_scenario $output_sub_dir $total_jobs | sed 's/Submitted batch job //')	
else
    echo "not evaluating iventory of outputs, per user specifications"
fi

#---------------------------------------------------------------------------------------------
# Step 7: Run an inventory to see what time periods failed in each run. This evaluates what 
# time periods failed in runs that successfully produced output csv files, so we can see if
# they are runs that we want to keep and analyze, or if they need to be re-run.
if [[ $run_gcam_fail_inventory -eq 1 ]]; then
	slurmoutname_gcf="./stdout/${gcam_meta_scenario}.${output_sub_dir}.failedruns.%A.out"
    echo "execute script to check *.out files to see what time periods failed in every run"
    InventoryDir="${repo_path}relationships/gcam/scripts/"
    InventoryFile="run-gcam-fail-inventory.sh"
    InventoryFpath="$InventoryDir$InventoryFile"	
    echo "sbatch -A $acct -p $partition --output=$slurmoutname_gcf $InventoryFpath $repo_path $gcam_meta_scenario $output_sub_dir $total_jobs"
	jid_gcf_inventory=$(sbatch -A $acct -p $partition --output=$slurmoutname_gcf $InventoryFpath $repo_path $gcam_meta_scenario $output_sub_dir $total_jobs | sed 's/Submitted batch job //')	
else
    echo "not evaluating iventory of outputs, per user specifications"
fi

#---------------------------------------------------------------------------------------------
# Step 8: If necessary, rename csv out files so that the scenario name in the filename matches 
# the scenario that corresponds to the contents of the file.
if [[ $rename_files -eq 1 ]]; then
    echo "execute script to rename csv output files"
	slurmoutname_rename="./stdout/${gcam_meta_scenario}.${output_sub_dir}.renamefiles.%A.out"
    ScriptDir="${repo_path}relationships/gcam/scripts/"
    ScriptFile="run-rename-file.sh"
    RenameFpath="$ScriptDir$ScriptFile"	
    echo "sbatch -A $acct -p $partition --output=$slurmoutname_rename $RenameFpath $repo_path $gcam_meta_scenario $output_sub_dir"
	jid_rename_files=$(sbatch -A $acct -p $partition --output=$slurmoutname_rename $RenameFpath $repo_path $gcam_meta_scenario $output_sub_dir | sed 's/Submitted batch job //')	
else
    echo "not renaming files, per user specifications"
fi
