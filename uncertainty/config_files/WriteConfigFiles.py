...
base_dir = 'C:/Users/twild/all_git_repositories/IDB_RDM/'
base_pic_dir = '../input/'
base_config_file = 'configuration_LAC.xml'
tree=ET.parse(base_dir + base_config_file)
...
for key in X_fac:
    if key not in ['climate_scen']:
        for item in X_fac[key]:
            factor_files[X_fac.keys().index(key)].append(base_pic_dir + '/rdm/' + key + '_' + item + '.xml')
    else:
...
for row in range(len(factor_file_list)):
    ...
    root[2][0].text = 'uncertainty_' + scen  # Scenario name
    ...
    tree.write(base_dir + 'configuration_LAC_' + scen + '.xml')  # Write XML file
