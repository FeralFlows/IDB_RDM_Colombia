...
DOE_DF = pd.DataFrame(columns = [std_cols + XL_fac.keys()],
    ...
    for key in XL_fac:
        if key not in ['climate_scen']:
            for item in XL_fac[key]:
                factor_files[XL_fac.keys().index(key)].append(base_pic_dir + 'idb_5.3/rdm/XL_category_files/' + key + '_' + item +
                                                              '.xml')
                factor_names[XL_fac.keys().index(key)].append(key)
                factor_levels[XL_fac.keys().index(key)].append(item)
        else:
