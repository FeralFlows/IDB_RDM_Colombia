...
GWP<-read.csv('D:/RDM/Colombia-RDM-Analysis-master/metrics/GWP.csv', header = TRUE, sep = ",", dec = ".")
#Import Correction factors
CF<-read.csv('D:/RDM/Colombia-RDM-Analysis-master/metrics/CorreccionFactor.csv', header = TRUE, sep = ",", dec = ".")
#Import deforestation emission trajectory
ghgtraj<-read.csv('D:/RDM/Colombia-RDM-Analysis-master/metrics/GHG traj.csv') %>%
  ##BY 12-17-2020
...
#####VKT
load_factors <- read.csv('D:/RDM/Colombia-RDM-Analysis-master/metrics/L254.StubTranTechLoadFactor.csv', skip=1) %>% filter(region == "Colombia")  
...
base_dir <- c('D:/RDM/runs_126_12_22_2020/query_proj_12_22_2020/')
export_dir <- c('D:/RDM/runs_126_12_22_2020/output_color/')
#Query names in folder
qries<-list.files(base_dir)

source('D:/RDM/Colombia-RDM-Analysis-master/metrics/RDM_plotting.R')
...
#  prj <- loadProject(prj_path)
  ...
  if(qry=="nonCO2 emissions by sector.proj"){
   ...
   #####GHG Emissions 
    ...
    Metrics<-rbind(as_tibble(CO2),as_tibble(nonco2),as_tibble(GHG),as_tibble(TCO2eqAgri),as_tibble(CO2eqFFI),as_tibble(TCO2eqAg),as_tibble(TCO2eqBk),as_tibble(TCO2eqComm),
                   ...
                   as_tibble(TCO2eqPower),as_tibble(TCO2eqRef),as_tibble(TCO2eqWst),as_tibble(N2O),as_tibble(CH4),as_tibble(NOx),as_tibble(SO2),as_tibble(TCO2eqTrn)
                   ,as_tibble(TCO2eqAFOLU),as_tibble(deforestarea),as_tibble(LUCcost))
    select_exps <- c('42', '58')
  ...
  if(qry=="Electricity generation by aggregate technology.proj"){
    ...
    Metrics<-rbind(as_tibble(PowGen),as_tibble(PowGenHydroGW),as_tibble(PowGenRenewGW),as_tibble(PowGenRenewIntGW),as_tibble(PowGenHydro),as_tibble(PowGenRenew),as_tibble(PowGenRenewInt))
  ...
  if(qry=="elec prices by sector.proj"){
    ...
    line_plot(electricity3, fig_path, plot_scens, y_lbl=y_lbl, x_lbl=x_lbl, title=title, x_min=x_min, x_max=x_max, legend_on=FALSE, gray_ribbon = TRUE,  plot_by_select_experiment=select_exps)
  ...
  if(qry=="total final energy by aggregate sector.proj"){
        ...
        line_plot(FinalEnergy, fig_path, plot_scens, y_lbl=y_lbl, x_lbl=x_lbl, title=title, x_min=x_min, x_max=x_max, legend_on=FALSE, gray_ribbon = TRUE)
        rm(FinalEnergy)
  }
  ...
  if(qry=="building final energy by fuel.proj"){
        ...
        line_plot(TradBio, fig_path, plot_scens, y_lbl=y_lbl, x_lbl=x_lbl, title=title, x_min=x_min, x_max=x_max, legend_on=FALSE, gray_ribbon = TRUE)
  ...
  line_plot(elecPrice1, fig_path, plot_scens, y_lbl=y_lbl, x_lbl=x_lbl, title=title, x_min=x_min, x_max=x_max, legend_on=FALSE, gray_ribbon = TRUE)
  rm(elecPrice1)
  ...
  if(qry=="elec gen by region (incl CHP).proj"){
    ...
    line_plot(Elecgen, fig_path, plot_scens, y_lbl=y_lbl, x_lbl=x_lbl, title=title, x_min=x_min, x_max=x_max, legend_on=FALSE, gray_ribbon = TRUE)
    rm(Elecgen)
  ...
  if(qry=="transport final energy by mode and fuel.proj"){
    ...
    #####US$ per liter
    costpollutant <- read.csv('D:/RDM/Colombia-RDM-Analysis-master/metrics/Cost of local pollutants.csv') %>%
    ## BY 12-18-2020
  ...
  if(qry=="primary energy consumption by region (avg fossil efficiency).proj"){
    ...
    Metrics<-rbind(as_tibble(all),as_tibble(oilp),as_tibble(gas),as_tibble(coalp),as_tibble(renewablesp),as_tibble(biomassp),as_tibble(hydrop)
  ...
  if(qry=="building service output by service.proj"){
    ...
    Metrics<-rbind(as_tibble(bldoutput1),as_tibble(bldoutput3), as_tibble(Build))
    for (i in seq(length(unique(Metrics$Metric)))) {
      M<-Metrics%>% filter(Metric %in% c(unique(Metrics$Metric)[i]))
      plot_df <- M
      fig_path <- c(paste(export_dir,unique(Metrics$Metric)[i],'.png',sep = ""))
  ...
  if(qry=="refined liquids production by subsector.proj"){
    ...
    for (i in seq(length(unique(Metrics$Metric)))) {
      ...
      y_lbl <- paste(M$Metric[1],' (',M$Units[1],')',sep = "")
      line_plot(plot_df, fig_path, plot_scens, plotXLfacs, y_lbl=y_lbl, x_lbl=x_lbl, x_min=x_min, x_max=x_max,title=title, legend_on=FALSE, gray_ribbon = TRUE)}
  }
#}
