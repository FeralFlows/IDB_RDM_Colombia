  ...
  if(!is.null(gray_ribbon)){
    ...
    if(!is.null(distribution)){
      ...
      smry_final <- plot_df %>% 
        filter(year %in% year_list) %>% 
        group_by(year) %>% 
        summarize(quantile = scales::percent(c(0, 0.1, 0.25, 0.75, 0.9, 1)),
                  value = quantile(value, c(0, 0.1, 0.25,  0.75, 0.9, 1))) %>% 
        mutate(.width = case_when(quantile %in% c('0%', '100%') ~ 1,
        ...
        select(year, value, .lower, .upper, .width, .point, .interval)
