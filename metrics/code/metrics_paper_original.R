...
PowGenRenewInt <- PowGenRenewIntGW %>%
  left_join(TotalPowGen,
            ...
            suffix = c('.int', '.total')) %>%
  mutate(value = if_else(value.total == 0, 0, value.int/value.total * 100),
...
PowGenRenewPct <- PowGenRenewGW %>%
  ...
  mutate(value = if_else(value.total == 0, 0, value.renew/value.total * 100),
...
PowGenLowCarbPct <- PowGenLowCarb %>%
  ...
  mutate(value = if_else(value.total == 0, 0, value.lowcarb/value.total * 100),
...
PowGenHydro <- PowGenHydroGW %>%
  ...
  mutate(value = if_else(value.total == 0, 0, value.hydro/value.total * 100),
...
RPS_pct <- ren_RPS %>%
  left_join(total_RPS) %>%
  mutate(pct = if_else(total_value == 0, 0, ren_value / total_value))
...
biomass <- PriEne %>%
  filter(fuel %in% c("d biomass"), region %in% 'Colombia') %>%
  group_by(scenario, region, experiment, old_scen_name, year) %>%
  dplyr::summarise(value = sum(value)) %>%
...
biomassp <- biomass %>%
  ...
  mutate(value = if_else(value.y == 0, 0, (value.x / value.y) * 100),
...
## Traditional biomass
biomass_trad <- PriEne %>%
  filter(fuel %in% c("j traditional biomass"), region %in% 'Colombia') %>%
  group_by(scenario, region, experiment, old_scen_name, year) %>%
  dplyr::summarise(value = sum(value)) %>%
...
biomass_liq_ratio <- RefLiqProd %>%
  ...
  mutate(value = if_else(value.total == 0, 0, value.biomass/value.total)) %>%
...
BiomassTrans <- Reduce(function(...)
  ...
  mutate(value = if_else(total_trans == 0, 0, (bio_liq + bio_elec + bio_gas)/total_trans * 100),
...
ElecPctFinalEneFuel <- ElecFinalEneFuel %>%
  left_join(TotalFinalEneFuel, by = c("scenario", "region", "Units", "year", "experiment", "old_scen_name"),
            suffix = c(".elec", ".total")) %>%
  mutate(value = if_else(value.total == 0, 0, value.elec / value.total * 100),
         Units = "%",
...
HydrogenPctFinalEneFuel <- HydrogenFinalEneFuel %>%
  ...
  mutate(value = if_else(value.total == 0, 0, value.hydrogen / value.total * 100),
         Units = "%",
...
BiomassFinalEne <- Reduce(function(...)
  ...
  dplyr::rename(bio_liq = value.x,
         ...
         bio_final = value.y.y,
         total_final = value) %>%
  mutate(value = if_else(total_final == 0, 0, (bio_liq + bio_elec + bio_gas + bio_final)/total_final * 100),
...
## Colombia oil
oil <- PriEne %>%
  filter(fuel %in% c("a oil", 'traded oil'), region %in% 'Colombia') %>%
  group_by(scenario, region, experiment, old_scen_name, year) %>%
  dplyr::summarise(value = sum(value)) %>%
  mutate(Units = "EJ",
...
oilp <- oil %>%
  left_join(all, by = c("scenario", "region", "experiment", "old_scen_name", "year")) %>%
  mutate(value = if_else(value.y == 0, 0, (value.x / value.y) * 100), Units = "%", value.x = NULL, value.y =
           NULL, Units.x = NULL, Units.y = NULL, Metric = "ColombiaOil Primary Energy Consumption") %>%
  select(scenario, region, experiment, old_scen_name, year, value, Metric, Units)

## Colombia natural gas
gas <- PriEne %>%
  filter(fuel %in% c("b natural gas", 'traded natural gas'), region %in% 'Colombia') %>%
  group_by(scenario, region, experiment, old_scen_name, year) %>%
  dplyr::summarise(value = sum(value)) %>%
  mutate(Units = "EJ",
...
gasp <- gas %>%
  left_join(all, by = c("scenario", "region", "experiment", "old_scen_name", "year")) %>%
  mutate(value = if_else(value.y == 0, 0, (value.x / value.y) * 100), Units = "%", value.x = NULL, value.y = NULL,
         Units.x = NULL, Units.y = NULL, Metric = "Colombia Gas Primary Energy Consumption") %>%
  select(scenario, region, experiment, old_scen_name, year, value, Metric, Units)
...
GlobalOil <- PriEne %>% 
  filter(fuel %in% c("a oil", 'traded oil')) %>%
  group_by(scenario, experiment, old_scen_name, year) %>%
  dplyr::summarise(value = sum(value)) %>%
  mutate(Units = "EJ",
  ...
  select(scenario, region, experiment, old_scen_name, year, value, Metric, Units)
GlobalNG <- PriEne %>% 
  filter(fuel %in% c("b natural gas", 'traded natural gas')) %>%
  group_by(scenario, experiment, old_scen_name, year) %>%
  dplyr::summarise(value = sum(value)) %>%
  mutate(Units = "EJ",
...
CongestionPass_2015 <- bind_rows(CongestionDieselPass_2015,
  ...
  mutate(value = if_else(value.mill_pass_km == 0, 0, value.USD/value.mill_pass_km),
...
CongestionFreight_2015 <- Freight_2015 %>%
  ...
  mutate(value = if_else(value.mill_ton_km == 0, 0, value.USD/value.mill_ton_km),
...
region_net_trade_volume <- region_imports %>%
  full_join(region_exports, by = c("region", "Units", "scenario", "old_scen_name", "experiment", "sector", "year"), suffix = c(".imports", ".exports")) %>%
...
region_net_trade_value <- region_net_trade_volume %>%
  left_join(region_import_prices, by = c("scenario", "old_scen_name", "experiment", "region", "sector", "year")) %>%
  select(-Units.x, -Units.y) %>%
  left_join(region_export_prices, by = c("scenario", "old_scen_name", "experiment", "region", "sector", "year")) %>%
  mutate_if(is.numeric, ~replace(., is.na(.), 0)) %>%
...
region_total_net_trade_value <- region_net_trade_value %>%
  group_by(scenario, region, old_scen_name, experiment, year, Units) %>%
...
region_domestic_value <- region_domestic %>%
  filter(year %in% c(seq(2015, 2050, 5))) %>%
  left_join(region_domestic_prices, by = c("scenario", "old_scen_name", "experiment", "region", "sector", "year")) %>%
...
region_production_value <- region_domestic_value %>%
  full_join(region_net_trade_value, by = c("scenario", "old_scen_name", "experiment", "region", "sector", "year", "Units")) %>%
  mutate_if(is.numeric, ~replace(., is.na(.), 0)) %>%
