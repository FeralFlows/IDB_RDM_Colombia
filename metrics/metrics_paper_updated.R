...
TotalPowGen <- ElecGenAggTech %>%
  ...
  mutate(Metric = "Total power gen.", Units = "Thous GWh")
...
biomassp <- biomass %>%
  ...
  mutate(value = (value.x / value.y) * 100, Units = "%", value.x = NULL, value.y =
           NULL, Units.x = NULL, Units.y = NULL, Metric = "Biomass share of primary energy", Units = "%") %>%
  filter(year %in% c(2010:2050)) %>%
  select(scenario, region, experiment, old_scen_name, year, value, Metric, Units)
...
# TO-DO: add CART for investment plots (Done!)
...
oilprice <- PriceAllMarkets %>%
  filter(market %in% c("globalcrude oil"), year %in% c(2010:2050)) %>%
  group_by(scenario, region, experiment, old_scen_name, year) %>%
  dplyr::summarise(value = sum(value)) %>%
...
ngprice <- PriceAllMarkets %>%
  filter(market %in% c("globalnatural gas"), year %in% c(2010:2050)) %>%
  group_by(scenario, region, experiment, old_scen_name, year) %>%
  dplyr::summarise(value = sum(value)) %>%
